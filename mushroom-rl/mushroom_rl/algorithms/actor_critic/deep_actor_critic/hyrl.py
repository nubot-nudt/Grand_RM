...
# HyRL: Boosted Hybrid RL (https://doi.org/10.1109/LRA.2022.3188109)
...
class HyRL(DeepAC):
    """
    HyRL with a Hybrid action space (A sequential discrete approximator takes as input the
    continous action and outputs the discrete part of the action)
    HyRL继承自DeepAC类，实现了一个具有混合动作空间的DRL算法
    ...
    def __init__(self, mdp_info, actor_mu_params, actor_sigma_params, actor_discrete_params,
        ...
        policy = HyRLPolicy(actor_mu_approximator,
        ...
        self.console_logger = ConsoleLogger(log_name='')
        # self.console_logger.info("初始化HyRL类, batch_size=%s, initial_replay_size=%d"%(self._batch_size, initial_replay_size))
    ...
    def fit(self, dataset):
        # self.console_logger.info("HyRL中调用fit函数, len(dataset)=%d"%(len(dataset)))
