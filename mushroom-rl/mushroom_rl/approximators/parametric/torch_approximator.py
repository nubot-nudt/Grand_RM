...
class TorchApproximator(Serializable):
    ...
    """
    def __init__(self, network, input_shape, output_shape, use_graph=False, optimizer=None,
    ...
    def predict(self, *args, output_tensor=False, **kwargs):
        ...
        if not self._use_cuda:
            ...
            if not hasattr(self, '_use_graph'):
            ...
            if not self._use_graph:
                ...
                """
                args=
                (array([[ 3.6355877 , -2.1503048 ,  1.0855315 ,  0.76360315,  0.31620562,
                        -0.56034875, -0.05416315]], dtype=float32),)
                torch_args=
                [tensor([[ 3.6356, -2.1503,  1.0855,  0.7636,  0.3162, -0.5603, -0.0542]],
                       device='cuda:0')]
                """

                """
                args=
                (array([[0.8903007507324219, 4.782650947570801, 1.316664457321167,
                        0.31930896639823914, 0.5453559160232544, -0.26483142375946045,
                        0.728349506855011,
                ...
                torch_args=
                [tensor([[ 0.8903,  4.7827,  1.3167,  0.3193,  0.5454, -0.2648,  0.7283]],
                       device='cuda:0'), Graph(num_nodes=1, num_edges=0,
                      ndata_schemes={'h': Scheme(shape=(33,), dtype=torch.float32)}
                      edata_schemes={'rel_type': Scheme(shape=(), dtype=torch.float32), 'norm': Scheme(shape=(1,), dtype=torch.float32)})]
                """
                for x in args:
                    if isinstance(x[0, -1], DGLGraph):
                        if x.shape[0] == 1:
                            torch_args.append(torch.from_numpy(x[:, :-1].astype(np.float32)).cuda())
                            torch_args.append(x[0][-1].to('cuda:0'))
                        else:  # 这里尤其需要注意对batch类型数据集的处理
                            torch_args.append(torch.from_numpy(x[:, :-1].astype(np.float32)).cuda())
                            batched_graph = dgl.batch(x[:, -1])
                            torch_args.append(batched_graph.to('cuda:0'))
                            # print("测试batch_graph=")
                            # print(batched_graph)
                            # input("测试batched_graph")
                    elif isinstance(x, np.ndarray):
                        torch_args.append(torch.from_numpy(x.astype(np.float32)).cuda())
    ...
    def _compute_batch_loss(self, batch, use_weights, kwargs):
        ...
        if not self._use_cuda:
            ...
            if not self._use_graph:
                ...
                torch_args = []
                for x in batch:
                    if x.ndim == 2 and isinstance(x[0, -1], DGLGraph):
                        if x.shape[0] == 1:
                            torch_args.append(torch.from_numpy(x[:, :-1].astype(np.float32)).cuda())
                            torch_args.append(x[0][-1].to('cuda:0'))
                        else:  # 这里尤其需要注意对batch类型数据集的处理
