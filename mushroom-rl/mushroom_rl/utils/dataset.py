...
def parse_dataset(dataset, use_graph=False, features=None):
    """
    ...
    # 创建一个具有相同维度的np.object类型的数组
    if use_graph:
        state = np.empty((len(dataset),) + shape, dtype=np.object)
    else:
        state = np.ones((len(dataset),) + shape)
    action = np.ones((len(dataset),) + dataset[0][1].shape)
    reward = np.ones(len(dataset))
    if use_graph:
        next_state = np.empty((len(dataset),) + shape, dtype=np.object)
    else:
