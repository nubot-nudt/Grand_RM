...
def spawn_obstacle(name, prim_path, device):
    ...
    obj = GeometryPrim(
        ...
        # JL修改：这一部分必须是[1.0, 0.0, 0.0, 0.0]正向的显示就好
...
def setup_tabular_scene(self, obstacles, tabular_obstacle_mask, grasp_objs, obstacles_dimensions, grasp_objs_dimensions,
    ...
    for idx, grasp_obj in enumerate(grasp_objs[::-1]):
        real_idx = len(grasp_objs) - 1 - idx
        grasp_obj_z_to_ground = - grasp_objs_dimensions[real_idx][0, 2]
        ...
        while (1):  # Be careful about infinite loops!
            ...
            """改变随机化测试"""
            # input("idx=%d, name=%s"%(idx, grasp_obj.name))
            grasp_obj.set_world_pose(position=torch.tensor([0.0, 0.0, 0.0], device=device),
            ...
            grasp_obj_aabbox = self._scene.compute_object_AABB(grasp_obj.name)
            ...
            if idx == 0:
                grasp_obj_x = tab_x + 0.4
                grasp_obj_y = tab_y - 0.0
            elif idx == 1:
                grasp_obj_x = tab_x - 0.1
            ...
            # input("grasp_obj_z=%s" % grasp_obj_z)

            if idx == 3:
                grasp_obj_z += 0.8

            grasp_obj_position = [grasp_obj_x, grasp_obj_y, grasp_obj_z]
            grasp_obj.set_world_pose(position=torch.tensor(grasp_obj_position, dtype=torch.float, device=device),
                                           orientation=euler_angles_to_quats(
            ...
            if (idx > 3 and overlap):
                ...
                objects_dimensions.append(grasp_objs_dimensions[real_idx])
                break
    ...
    for idx, _ in enumerate(obstacles):
        ...
        while (1):  # Be careful about infinite loops!
            ...
            #  JL修改：机器人不再是从[0, 0]开始出发
            # obst_r = np.random.uniform(robot_radius + np.max(obst_xyz_size[0:2]),
            #                            world_xy_radius)  # taking max xy size margin from robot
            obst_r = np.random.uniform(0.0, world_xy_radius)
    ...
    goal_position[2] = (grasp_obj_aabboxes[goal_obj_index][1, 2])
