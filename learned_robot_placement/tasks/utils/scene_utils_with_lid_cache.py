...
def setup_tabular_scene(self, obstacles, tabular_obstacle_mask, grasp_objs, obstacles_dimensions, grasp_objs_dimensions,
    ...
    obstacles[tab_index].set_world_pose(position=torch.tensor(tab_position, dtype=torch.float, device=device),
                                        orientation=torch.tensor([0.707106, 0.707106, 0.0, 0.0],
        ...
        while (1):  # Be careful about infinite loops!
            ...
            grasp_objs[idx].set_world_pose(position=torch.tensor([0.0, 0.0, 0.0], device=device),
                                           orientation=euler_angles_to_quats(
                                               torch.tensor([[-torch.pi / 2, 0, grasp_obj_yaw]], device=device))[
            ...
            if idx == 0:
                grasp_obj_x = tab_x
                grasp_obj_y = tab_y + 0.3
            elif idx == 1:
                grasp_obj_x = tab_x + 0.5
                grasp_obj_y = tab_y - 0.05
            elif idx == 2:
                grasp_obj_x = tab_x - 0.48
                grasp_obj_y = tab_y - 0.05
            elif idx == 3:
                grasp_obj_x = tab_x
                grasp_obj_y = tab_y
            else:
                grasp_obj_x = tab_x + np.random.uniform((-tab_xyz_size[0] + 0.4 - grasp_obj_aabbox[0, 0]) / 2.0,
                                                        (tab_xyz_size[0] - 0.4 - grasp_obj_aabbox[1, 0]) / 2.0)
                grasp_obj_y = tab_y + np.random.uniform((-tab_xyz_size[1] - grasp_obj_aabbox[0, 1]) / 2.0,
                                                        (tab_xyz_size[1] - 0.5 - grasp_obj_aabbox[1, 1]) / 2.0)
            """改变随机化测试"""
            ...
            grasp_objs[idx].set_world_pose(position=torch.tensor(grasp_obj_position, dtype=torch.float, device=device),
                                           orientation=euler_angles_to_quats(
                                               torch.tensor([[-torch.pi / 2, 0, grasp_obj_yaw]], dtype=torch.float,
    ...
    obstacles[tab_index].set_world_pose(
        orientation=euler_angles_to_quats(torch.tensor([[torch.pi / 2, 0, tab_yaw]], device=device))[0])
    for idx, _ in enumerate(grasp_objs):
        if idx < 4:
        ...
        grasp_objs[idx].set_world_pose(position=torch.tensor(object_positions[idx], dtype=torch.float, device=device),
                                       orientation=euler_angles_to_quats(
                                           torch.tensor([[-torch.pi / 2, 0, object_yaws[idx]]], device=device))[0])
    ...
    for idx, _ in enumerate(obstacles):
        ...
        while (1):  # Be careful about infinite loops!
            ...
            obstacles[idx].set_world_pose(position=torch.tensor(obst_position, device=device),
                                          orientation=euler_angles_to_quats(
                                              torch.tensor([[torch.pi / 2, 0, obst_yaw]], device=device))[0])
            # compute new AxisAligned bbox
            self._scene._bbox_cache.Clear()
    ...
    for idx in range(len(object_positions)):
        ...
        for obstacle in obstacles:
            cache_obstacles_loc.append(obstacle.get_world_pose())
        cache_objects_loc = []
        for grasp_obj in grasp_objs:
            cache_objects_loc.append(grasp_obj.get_world_pose())
        cache_loc.append(cache_obstacles_loc)
        cache_loc.append(cache_objects_loc)
        # 记录历史存储的数据

    return grasp_objs[goal_obj_index], goal_pose, object_oriented_bboxes, tab_xyz_size, tab_position, tab_yaw, object_positions, cache_loc


def resume_cache(obstacles, grasp_objs, cache_loc):
    cnt = 0
    for obstacle in obstacles:
        obstacle.set_world_pose(position=cache_loc[0][cnt][0], orientation=cache_loc[0][cnt][1])
        cnt += 1

    cnt = 0
    for grasp_obj in grasp_objs:
        grasp_obj.set_world_pose(position=cache_loc[1][cnt][0], orientation=cache_loc[1][cnt][1])
        cnt += 1
